...
index_dir = '/net/seq/data2/projects/sabramov/SuperIndex/clean_index_clustering/dnase_3501/stratified_sampling.downsampled.0326/only1_downsampled/all_samples/output/'
data_dir = '/net/seq/data2/projects/acote/projects/encode4/4025_index/subsets/new_normalization_all/output/'
results_dir = '/net/seq/data2/projects/acote/projects/encode4/4025_index/deseq/all_new_normalization/output/'
...
run_deseq <- function(row){
    ...
    data_file <- paste0(results_dir, cluster_group,'_deseq_wnorm_postdeseq.rds')
    saveRDS(dds, data_file)
    
    res <- results(dds, parallel = TRUE)
    data.frame(res) %>% write.csv(file = paste0(results_dir, cluster_group, '_deseq_wnorm_results.tsv'),sep='\t')
